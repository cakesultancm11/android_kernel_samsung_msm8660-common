...
#define CONFIG_LCD_NOTIFY
#define DHD_USE_LCD_NOTIFY
...
#else
#if defined(CONFIG_LCD_NOTIFY) && defined(DHD_USE_LCD_NOTIFY)
#include <linux/lcd_notify.h>
#endif /* defined(CONFIG_LCD_NOTIFY) && defined(DHD_USE_LCD_NOTIFY) */
...
#if defined(CONFIG_LCD_NOTIFY) && defined(DHD_USE_LCD_NOTIFY)
static struct notifier_block dhd_lcd_notif;
#endif /* CONFIG_LCD_NOTIFY && DHD_USE_LCD_NOTIFY */
...
#if defined(CONFIG_LCD_NOTIFY) && defined(DHD_USE_LCD_NOTIFY)
static void dhd_early_suspend(struct notifier_block *h)
{
...
static void dhd_late_resume(struct notifier_block *h)
{
	struct dhd_info *dhd = NULL;
	DHD_TRACE_HW4(("%s: enter\n", __FUNCTION__));
...
static int dhd_lcd_notifier_callback(struct notifier_block *this,
				unsigned long event, void *data) {
	pr_debug("%s: event = %lu\n", __func__, event);

	switch (event) {
	case LCD_EVENT_ON_START:
		dhd_late_resume(this);
		break;
	case LCD_EVENT_ON_END:
		break;
	case LCD_EVENT_OFF_START:
		dhd_early_suspend(this);
		break;
...
#if defined(CONFIG_LCD_NOTIFY) && defined(DHD_USE_LCD_NOTIFY)
	dhd_lcd_notif.notifier_call = dhd_lcd_notifier_callback;
	if (lcd_register_client(&dhd_lcd_notif) != 0) {
		pr_err("%s: Failed to register lcd callback\n", __func__);
//		ret = -EINVAL;
		lcd_unregister_client(&dhd_lcd_notif);
		goto fail;
	}
